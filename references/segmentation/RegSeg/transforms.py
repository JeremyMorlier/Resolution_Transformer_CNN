...
class ToTensor2(object) :
    def __call__(self, image, target):
        image_transforms = TV.Compose([TV.ToImage(), TV.ToDtype(torch.float32)])
...
class CenterCropLabel(object):
    ...
    def __call__(self, image, label):
        ...
        crop_fun = T.CenterCrop((crop_h, crop_w))
        image_cropped = crop_fun(image)
        label_cropped = crop_fun(label)
...
class GreyScale(object):
    def __init__(self):
        self.done = True
    def __call__(self, image, label):
        grey_image = image[0, :, :].unsqueeze(0)
        return grey_image,label

class RandomCenterLabelCrop(object):
    def __init__(self, crop_w, crop_h, flip_prob):
        self.flip_prob = flip_prob
        self.crop_w = crop_w
        self.crop_h = crop_h
    def __call__(self, image, label):
        ...
        # At random time, the image and label will be centered around the segmented area or not
        if random.random() < self.flip_prob:
            return image, label
        else :
            img_w,img_h=image.size
