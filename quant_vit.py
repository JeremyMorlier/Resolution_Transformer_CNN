# Evaluate pretrained ViT-S with and without quantization
import os, sys
...
def get_named_model(model_name, device=None, path=None) :

    list_args = model_name.split("_")
    patch_size, num_layers, num_heads, hidden_dim, mlp_dim, model_img_size = [int(element) for element in list_args[2:]]
    model = get_model("vit_custom", weights=None, num_classes=1000, patch_size=patch_size, num_layers=num_layers, num_heads=num_heads, hidden_dim=hidden_dim, mlp_dim=mlp_dim, image_size=model_img_size)
...
def init_log(model_dir) :
    filepath = os.path.join(model_dir, "log_quant.txt")
...
if __name__ == "__main__" :
    ...
    for model_name in models :
        if model_name in valid_models and model_name not in log["evaluated models"]:
            ...
            val_crop_resolution = args.model_img_size
            model = get_named_model(model_name, device, path)
            ...
            log[model_name]["no_quant"] = {}
            log[model_name]["no_quant"]["model_size"] = model_size
            log[model_name]["no_quant"]["acc1"] = results[0]
            log[model_name]["no_quant"]["flops"] = flops
            log[model_name]["no_quant"]["act_memory"] = memory
            log[model_name]["no_quant"]["total_memory"] = total_memory   
            ...
            results = evaluate(model_int8, val_loader, device)
            log[model_name]["args"] = args.__dict__
            log[model_name]["quant"] = {}
            log[model_name]["quant"]["model_size"] = model_size
            log[model_name]["quant"]["acc1"] = results[0]
            log[model_name]["quant"]["flops"] = flops
            log[model_name]["quant"]["act_memory"] = memory
            log[model_name]["quant"]["total_memory"] = total_memory 
